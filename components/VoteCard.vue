...
<template>
  <div class="youtube-card transition-all cursor-pointer relative animate-card-in max-w-lg mx-auto" :class="[
    isSelected ? 'z-10 scale-105' :
      isRejected ? '!opacity-50' :
        isDisabled ? 'opacity-70' : 'hover:bg-(--ui-yt-800)/25'
  ]" @click="handleSelect" v-if="metadata">
    <!-- Thumbnail container with overlay -->
    <div class="relative rounded-xl overflow-hidden mb-3.5">
      <img :src="`${battleType === 'title' ? metadata.thumbnailUrl : '/api/' + option.content}`"
        class="w-full aspect-video object-cover" :class="battleType === 'title' ? 'sepia blur-sm' : ''"
        :alt="option.content" />
    ...
    <div class="flex gap-3">
      ...
      <div class="flex-shrink-0 relative rounded-full overflow-hidden" style="width: 40px; height: 40px;">
        ...
        /> -->
        <img src="/micode.jpg" alt="micode" class="absolute inset-0 w-full h-full object-cover"
          :class="battleType === 'title' ? 'sepia blur-[2px]' : ''" />
      </div>


      <!-- Title and metadata text-overflow-ellipsis ? -->
      <div class="flex-grow">
        <!-- Video title -->
        <h3
          class="text-(--ui-yt-200) text-[1.2rem] overflow-hidden line-clamp-2 leading-snug max-h-[4.4rem] font-medium whitespace-normal"
          style="display: -webkit-box; -webkit-box-orient: vertical;" :title="option.content">
      ...
      <div class="cursor-pointer self-start mt-[2px]" :class="battleType === 'title' ? 'blur-[1px]' : ''">
        <UIcon name="i-lucide-more-vertical" class="h-5 w-5 text-(--ui-yt-200)/90" />
      </div>
    </div>

    <!-- Message stamp for voting feedback -->
    <div v-if="displayStamp && stampMessage && stampPosition" class="absolute opacity-0 animate-stamp-in z-20" :style="{
      top: stampPosition.top,
      left: stampPosition.left,
      transform: `rotate(${stampPosition.rotate})`
    }">
      <div class="rounded-lg px-3 py-1 text-md font-bold border-2 shadow-xl"
